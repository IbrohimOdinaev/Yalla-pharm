...
public sealed class StaffTelegramNotificationService : IStaffTelegramNotificationService
{
  private const string ConfirmCallbackPrefix = "tgstaff:cnf:";
  private const string CancelCallbackPrefix = "tgstaff:cnc:";

  private readonly IAppDbContext _dbContext;
  private readonly IStaffTelegramBotApi _bot;
  private readonly StaffTelegramNotificationOptions _options;
  private readonly ILogger<StaffTelegramNotificationService> _logger;

  public StaffTelegramNotificationService(
    IAppDbContext dbContext,
    IStaffTelegramBotApi bot,
    IOptions<StaffTelegramNotificationOptions> options,
    ILogger<StaffTelegramNotificationService> logger)
  {
  ...
  public async Task<StaffTelegramRecipientsResponse> GetRecipientsAsync(
    ...
    CancellationToken cancellationToken = default)
  {
    ...
    var recipients = await _dbContext.StaffTelegramRecipients
      .AsNoTracking()
      .Where(x => x.PharmacyWorkerId == pharmacyWorkerId && x.IsActive)
      .OrderByDescending(x => x.CreatedAtUtc)
      .Select(x => ToResponse(x))
      .ToListAsync(cancellationToken);

    return new StaffTelegramRecipientsResponse { Recipients = recipients };
  }
    ...
    CancellationToken cancellationToken = default)
  {
    ...
    var session = new TelegramAuthSession(nonce, expiresAtUtc, pharmacyWorkerId);
    _dbContext.TelegramAuthSessions.Add(session);
    await _dbContext.SaveChangesAsync(cancellationToken);

    var botUsername = NormalizeBotUsername(_options.BotUsername);
    var payload = Uri.EscapeDataString($"staff_notify_{nonce}");
    ...
    CancellationToken cancellationToken = default)
  {
    ...
    var session = await _dbContext.TelegramAuthSessions
      .AsNoTracking()
      .FirstOrDefaultAsync(x => x.Nonce == nonce && x.InitiatingUserId == pharmacyWorkerId, cancellationToken);
  ...
  public async Task<StaffTelegramRecipientResponse> CompleteLinkAsync(
    Guid pharmacyWorkerId,
    string nonce,
    CancellationToken cancellationToken = default)
  {
    ...
    var recipient = await _dbContext.StaffTelegramRecipients
      .AsTracking()
      .FirstOrDefaultAsync(
        x => x.PharmacyWorkerId == pharmacyWorkerId && x.ChatId == session.ConfirmationChatId.Value,
        cancellationToken);

    if (recipient is null)
    {
      recipient = new StaffTelegramRecipient(
        pharmacyWorkerId,
      ...
      _dbContext.StaffTelegramRecipients.Add(recipient);
    }
    else
    {
    ...
    CancellationToken cancellationToken = default)
  {
    ...
    var recipient = await _dbContext.StaffTelegramRecipients
      .AsTracking()
      .FirstOrDefaultAsync(x => x.Id == recipientId && x.PharmacyWorkerId == pharmacyWorkerId, cancellationToken)
    ...
    CancellationToken cancellationToken = default)
  {
    ...
    var promptText =
      "Подключить этот Telegram для уведомлений Yalla Pharm?\n\n" +
      $"Аккаунт: {displayName}{usernameLine}\n\n" +
      "На этот чат будут приходить новые заказы и запросы фармацевта.";
    ...
    CancellationToken cancellationToken = default)
  {
    ...
    await _bot.EditMessageTextAsync(
      chatId,
      messageId,
      "Готово. Вернитесь в профиль администратора, привязка завершится автоматически.",
      cancellationToken);
    await _bot.AnswerCallbackQueryAsync(callbackQueryId, "Подключено", showAlert: false, cancellationToken);

    _logger.LogInformation(
      "Staff Telegram link confirmed. Nonce={Nonce}, TgUserId={TgUserId}",
      nonce,
      telegramUserId);
  }
