...
/// <summary>
/// MinIO-backed implementation of <see cref="IManualLookupImageStorage"/>.
/// Same bucket as medicine and prescription imagery, but a distinct
/// `manual-lookups/yyyy/MM/dd/{guid}{ext}` prefix — keeps out-of-catalog
/// lookup photos isolated for retention / cleanup purposes.
/// </summary>
public sealed class MinIoManualLookupImageStorage : IManualLookupImageStorage
{
    private const string KeyPrefix = "manual-lookups";

    private readonly IMinioClient _minioClient;
    private readonly MinIoOptions _options;

    public MinIoManualLookupImageStorage(IOptions<MinIoOptions> options)
    {
      ...
      CancellationToken cancellationToken = default)
    {
        ...
        catch (Exception ex)
        {
            throw CreateMinIoOperationException(
              $"Failed to upload manual-lookup image to MinIO bucket '{_options.BucketName}'.",
              ex);
        }
        finally
        {
    ...
    public async Task<ManualLookupImageContent> GetContentAsync(
      string key,
      CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            output.Position = 0;
            return new ManualLookupImageContent
            {
                Content = output,
