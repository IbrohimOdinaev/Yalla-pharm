...
public sealed class SuperAdminTelegramBotUpdateHandler
{
  private const string StartCommandPrefix = "/start ";
  private const string SuperAdminNotifyDeeplinkPrefix = "superadmin_notify_";
  private const string ConfirmCallbackPrefix = "tgsuper:cnf:";
  private const string CancelCallbackPrefix = "tgsuper:cnc:";

  private readonly ISuperAdminTelegramNotificationService _service;
  private readonly ILogger<SuperAdminTelegramBotUpdateHandler> _logger;

  public SuperAdminTelegramBotUpdateHandler(
    ISuperAdminTelegramNotificationService service,
    ILogger<SuperAdminTelegramBotUpdateHandler> logger)
  {
  ...
  public async Task HandleAsync(TelegramUpdate update, CancellationToken cancellationToken = default)
  {
    ...
    {
      _logger.LogError(exception, "SuperAdmin Telegram bot update handler crashed. UpdateId={UpdateId}", update.UpdateId);
    }
  }
  ...
  private async Task HandleMessageAsync(TelegramMessage message, CancellationToken cancellationToken)
  {
    ...
    {
      _logger.LogInformation("SuperAdmin Telegram bot received /start without payload.");
      return;
    }

    var arg = text[StartCommandPrefix.Length..].Trim();
    if (!arg.StartsWith(SuperAdminNotifyDeeplinkPrefix, StringComparison.Ordinal))
    {
      _logger.LogInformation("SuperAdmin Telegram bot received /start with unsupported payload.");
      return;
    }

    var nonce = arg[SuperAdminNotifyDeeplinkPrefix.Length..];
    ...
    {
      _logger.LogWarning(
        "SuperAdmin Telegram bot received link start without chat/from. HasChat={HasChat}, HasFrom={HasFrom}",
