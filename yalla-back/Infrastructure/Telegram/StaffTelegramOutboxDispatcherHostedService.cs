...
public sealed class StaffTelegramOutboxDispatcherHostedService : BackgroundService
{
  private readonly IServiceScopeFactory _scopeFactory;
  private readonly StaffTelegramNotificationOptions _options;
  private readonly ILogger<StaffTelegramOutboxDispatcherHostedService> _logger;

  public StaffTelegramOutboxDispatcherHostedService(
    IServiceScopeFactory scopeFactory,
    IOptions<StaffTelegramNotificationOptions> options,
    ILogger<StaffTelegramOutboxDispatcherHostedService> logger)
  {
  ...
  protected override async Task ExecuteAsync(CancellationToken stoppingToken)
  {
    if (!_options.Enabled)
    {
      _logger.LogInformation("Staff Telegram outbox dispatcher is disabled by configuration.");
      return;
    }

    if (string.IsNullOrWhiteSpace(_options.BotToken))
    {
      _logger.LogWarning("Staff Telegram outbox dispatcher is enabled but BotToken is empty.");
      return;
    }
    ...
    _logger.LogInformation(
      "Staff Telegram outbox dispatcher started. PollIntervalSeconds={PollIntervalSeconds}, BatchSize={BatchSize}, MaxAttempts={MaxAttempts}",
  ...
  public async Task RunOnceAsync(CancellationToken cancellationToken = default)
  {
    try
    {
      ...
      var telegramBot = scope.ServiceProvider.GetRequiredService<IStaffTelegramBotApi>();
      ...
      var dueIds = await dbContext.StaffTelegramOutboxMessages
        .AsNoTracking()
      ...
      {
        var claimed = await dbContext.StaffTelegramOutboxMessages
          .Where(x =>
            x.Id == id
        ...
        var message = await dbContext.StaffTelegramOutboxMessages
          .AsTracking()
        ...
        catch (Exception exception)
        {
          ...
          if (isTerminal || message.AttemptCount >= maxAttempts)
          {
            ...
            _logger.LogWarning(
              exception,
              "Staff Telegram outbox dispatch permanently failed. MessageId={MessageId}, WorkerId={WorkerId}, ChatId={ChatId}, ErrorCode={ErrorCode}",
              message.Id,
              message.PharmacyWorkerId,
              message.ChatId,
              errorCode);
          }
          else
          {
            ...
            _logger.LogInformation(
              "Staff Telegram outbox dispatch transient error, retry scheduled. MessageId={MessageId}, ChatId={ChatId}, AttemptsSoFar={Attempt}, RetryAfterSeconds={Delay}, ErrorCode={ErrorCode}",
              message.Id,
              message.ChatId,
      ...
      if (processed > 0)
        _logger.LogInformation("Staff Telegram outbox dispatcher processed {Count} messages.", processed);
    }
    catch (OperationCanceledException)
    {
      throw;
    ...
    {
      _logger.LogError(exception, "Staff Telegram outbox dispatcher failed.");
    }
  }
    ...
    CancellationToken cancellationToken)
  {
    ...
    var removed = await dbContext.StaffTelegramOutboxMessages
      .Where(x =>
    ...
    if (removed > 0)
      _logger.LogInformation("Staff Telegram outbox retention cleanup removed {Count} messages.", removed);
  }
